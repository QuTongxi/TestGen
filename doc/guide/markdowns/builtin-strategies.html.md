...
Generates booleans.
text()
Generates unicode strings (i.e., instances of
str
). Can be constrained to ASCII withst.text(st.characters(codec="ascii"))
.lists()
...
.builds()
Generates instances of a class (or other callable) by specifying a strategy for each argument, like
...
st.sampled_from(["a", 1])
is roughly equivalent tost.just("a") | st.just(1)
.none()
Generates
None
