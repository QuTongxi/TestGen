...
For instance, suppose we write a test using `integers()`, but want to make sure we try a few special prime numbers every time we run the test. We can add these inputs with `@example`:
